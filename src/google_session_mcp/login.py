...
from . import config
from .browser import GOOGLE_HOME, LOGIN_HOST_MARKERS, _BASE_ARGS


async def _login(profile: Path) -> int:
    profile.mkdir(parents=True, exist_ok=True)
    print(f"Launching a visible browser with profile: {profile}")
    print("Log in to your CORPORATE Google account and open any Google service.")
    async with async_playwright() as pw:
        ...
        await asyncio.get_event_loop().run_in_executor(
            None,
            input,
            "\n>>> Finish login, make sure you can see Drive/Calendar/Gmail, then press Enter... ",
        )
        url = page.url
    ...
    if logged_in:
        ...
        print("You can now run the MCP server: google-browser-mcp serve")
        return 0
    print("\nDoes NOT look logged in (no SAPISID / still on login URL). Try again.")
    return 1
